...
int main(int argc, char **argv){
	...
	fprintf(stdout,"K=%"PRIvertex ", Connectivity=%"PRIu64 "\n", 0, init_conn);
    free_cc(scc);
    end_conn = init_conn;
	while( end_conn != 0 && k <= max_k ){
		critical_sets = get_digraph_critical_nodes_bruteforce( g, k, MAX_SETS, &end_conn, &tot_sets);
		//critical_sets = get_digraph_critical_nodes_bruteforce( g, k, 0, &end_conn, &tot_sets);
		int crt_sets = tot_sets < MAX_SETS ? tot_sets : MAX_SETS;
		for( int i=0; i<crt_sets; i++){
			fprintf(stdout, "\tCritical set number %d for K=%d\t-->\t",i,k);
			for(int j=0; j<k; j++){
				fprintf(stdout,"%"PRIvertex" ",critical_sets[i][j]);
			}
			fprintf(stdout, "\n");
		}
        free(critical_sets);
